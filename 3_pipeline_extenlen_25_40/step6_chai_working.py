...
"""Provide a list of seperated fasta file folder as input
"""
...
match0 = re.search(r"extendlen_(\\d+)_", fasta_basename)
match = re.search(r"cycle_(\\d+)_(\\d+).", fasta_basename)
sample_match = re.search(r"_sample(\\d+).", fasta_basename)

num0 = int(match0.group(1))
num1 = int(match.group(1))
num2 = int(match.group(2))
sample_number = int(sample_match.group(1)) if sample_match else None

base_name = f"extendlen_{{num0}}_cycle_{{num1}}_{{num2}}_sample{{sample_number}}"
...
def move_best_files(fasta_file, best_npz, best_cif, best_score):
    ...
    # Extract the current folder name (e.g., cycle_x_x_samplex)
    match0 = re.search(r"extendlen_(\d+)_", fasta_basename)
    match = re.search(r"cycle_(\d+)_(\d+).", fasta_basename)
    sample_match = re.search(r"_sample(\d+).", fasta_basename)
    num0 = int(match0.group(1))
