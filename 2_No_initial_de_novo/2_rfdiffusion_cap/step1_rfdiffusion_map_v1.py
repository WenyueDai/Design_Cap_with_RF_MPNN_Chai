...
REMOTE_DIR = "/rds/user/wd304/hpc-work/20241022_3ult_inpaint_morehelix333000"
LOCAL_FILE_PATH = "/home/eva/0_bury_charged_pair/2_No_initial_de_novo/2_rfdiffusion_cap/20241022_3ult_inpaint_morehelix333000"
...
LEEWAY = 10 # How much residue is allow to interact with N/C cap (in contact map), and in inpaint
HELIX_LENGTH = 7
GAP_LENGTH = 3

# ----- Step 1: Setup search space -----
cycle_list = range(1, 6)  # Cycle values from 1 to 5
extend_len_list = range(30, 41)  # Extension length from 30 to 40
# Numbers expressed in terms of PyMol residue numbering (i.e. starts at 1, not 0).
crop_NT_list = range(1, 4)  # N-terminal cropping from 1 (no crop) to 3 (max crop). You crop everything before this residue.
crop_CT_list = range(112, 115)  # C-terminal cropping from 112 (max crop) to 114 (no crop). You crop everything after this residue.
