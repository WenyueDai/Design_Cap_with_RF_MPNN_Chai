...
"""In this script use the pyrosetta converted protein structure for submission
"""

HPC_USER = "wd304@login-icelake.hpc.cam.ac.uk"
REMOTE_DIR = "/rds/user/wd304/hpc-work/20241022_3ult_noinpaint_tomek"
LOCAL_FILE_PATH = "/home/eva/0_bury_charged_pair/2_No_initial_de_novo/2_rfdiffusion_cap/20241022_3ult_noinpaint_tomek"
INPUT_PDB_SOURCE = "/home/eva/0_bury_charged_pair/2_No_initial_de_novo/2_rfdiffusion_cap/input/3ult_cleaned_renumbered_monomer_modified.pdb"
...
extend_len_list = range(25, 41)  # Extension length from 30 to 40
# Numbers expressed in terms of PyMol residue numbering (i.e. starts at 1, not 0).
crop_NT_list = range(1, 7)  # N-terminal cropping from 1 (no crop) to 3 (max crop). You crop everything before this residue.
crop_CT_list = range(109, 115)  # C-terminal cropping from 112 (max crop) to 114 (no crop). You crop everything after this residue.
...
def get_scaffoldguided(row):
    ...
    # Have to work with array indexes (i.e. starting at 0). All of the below are fed into a range() function.
    blocks = {"N-terminal": (0, row['extend_len'] + 1 + LEEWAY),
              #Removed temporarily  "Core": (row['extend_len'], chain_len - row['extend_len']),
              "C-terminal": (chain_len - (row['extend_len'] + LEEWAY), chain_len)}
    for (start, end) in blocks.values():
        for x, y in itertools.product(list(range(start, end)), repeat=2):
            block_adj[x, y] = 1
            
    for line in block_adj.numpy():
        print(" ".join(map(lambda x: str(int(x)), line)))
            
    # 0 = Helix, 1 strand, 2 loop, 3 Mask/Unknown
    pattern = [3] * chain_len
    # Randomly change some of the residues to be alpha helical. Still using 0-based indexing.
...
def write_task(row):
    command = ("/home/wd304/.conda/envs/SE3nv-cuda116/bin/python "
              ...
              f"contigmap.inpaint_seq=[A{row['crop_NT']}-{row['crop_NT'] + LEEWAY}/A{row['crop_CT'] - LEEWAY}-{row['crop_CT']}] "
